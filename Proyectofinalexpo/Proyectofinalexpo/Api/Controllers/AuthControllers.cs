...
namespace Api.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public AuthController(IUsuarioRepositorio usuarioRepositorio, IRolRepositorio rolRepositorio, IPacienteRepositorio pacienteRepositorio, IMedicoRepositorio medicoRepositorio, IConfiguration config)
        {
        ...
        public async Task<IActionResult> Login([FromBody] LoginRequest login)
        {
            ...
            var paciente = await _pacienteRepositorio.ObtenerPorUsuarioIdAsync(usuario.Id);
            var idPaciente = paciente?.Id;
            var medico = await _medicoRepositorio.ObtenerPorUsuarioIdAsync(usuario.Id);
            var idMedico = medico?.Id;
            if (nombreRol == "Medico")
            {
                return Ok(new { token, nombreRol, idUsuario = usuario.Id, idMedico });
            }
            else if (nombreRol == "Paciente")
            {
                return Ok(new { token, nombreRol, idUsuario = usuario.Id, idPaciente });
            }
            else
            {
                return Ok(new { token, nombreRol, idUsuario = usuario.Id });
            }
        }
